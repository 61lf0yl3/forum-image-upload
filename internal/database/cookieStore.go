...
import (
	...
	"forum-image-upload/internal/model"
)
...
// IsCookieInDB ...
func (d *Database) IsCookieInDB(cookie string) (bool, error) {
	if err := d.Open(); err != nil {
		return false, err
	}
	var i int64
	if err := d.db.QueryRow("SELECT userID FROM Sessions WHERE cookieValue = ?", cookie).Scan(&i); err != nil {
		return false, nil
	}
	return true, nil
}
